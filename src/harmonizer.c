...
harmonizer_process ( jack_nframes_t nframes, void *arg )
{
    ...
    for ( i = 0; i < 2; i++ )
    {
        ...
        in = jack_port_get_buffer ( _harmonizer_data.jack->input_ports[input_chan], nframes );
        out = jack_port_get_buffer ( _harmonizer_data.jack->output_ports[i], nframes );
        ...
        for (j = 0; j < nframes; ++j) {
            ...
            out[j] = (h1 * b1[jp * 2] + (1-h1) * b2[p + jp * 2]) * 8;
            // 1 octave lower
            // TODO align phase so there is no interference
            float h2 =  hann(t / 2);
            out[j] += (in[j / 2] * h2 + _harmonizer_data.prev_buf[(nframes + j) / 2] * (1-h2)) * 8;
        }
        // TODO save 2 buffers
        if (i == 1)
        memcpy ( _harmonizer_data.prev_buf, in, nframes * sizeof ( jack_default_audio_sample_t ) );
    }
    return 0;
}
