...
int init_io(jack_backend_t *jack) {
    ...
    for ( i = 0; i < 2; i++ )
    {
        ...
        jack->input_ports[i] = jack_port_register ( jack->client, port_name, JACK_DEFAULT_AUDIO_TYPE, JackPortIsInput, 0 );
        sprintf ( port_name, "output_%d", i + 1 );
        jack->output_ports[i] = jack_port_register ( jack->client, port_name, JACK_DEFAULT_AUDIO_TYPE, JackPortIsOutput, 0 );
...
int start_jack(jack_backend_t* jack) {
    ...
    fprintf(stderr, "port name = %s, port=%s\n", jack_port_name(jack->input_ports[i]), ports[i]);
        if ( jack_connect ( jack->client, ports[i], jack_port_name ( jack->input_ports[i] ) ) ) {
    ...
    fprintf(stderr, "port name = %s, port=%s\n", jack_port_name(jack->output_ports[i]), ports[i]);
        if ( jack_connect ( jack->client, jack_port_name ( jack->output_ports[i] ), ports[i] ) )
            fprintf ( stderr, "cannot connect input ports\n" );
    }
